...
def runxdsmoc(category_selected='all', subcategories={}, password=None):
    """
    This function will be removed, still there for compatibility with other tools
    Everything is on the config/run.py file
    """
    for pwd_dic in run_xdsmoc(category_selected=category_selected, subcategories=subcategories, password=password):
        yield pwd_dic
...
if __name__ == '__main__':
    ...
    PWrite.add_argument('-key', dest='key_pass', action='store', default=None,
    ...
    try:
        for r in runxdsmoc(category_selected=category, subcategories=subcategories,
                           password=args.get('password', None)):
            pass
    except KeyboardInterrupt:
        pass
    write_in_file(constant.stdout_result, args['key_pass'])
