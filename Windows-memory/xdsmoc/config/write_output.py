...
from xdsmoc.config.winstructure import string_to_unicode, char_to_int, chr_or_byte, python_version
from xdsmoc.config.encrypt_file import Aescrypt
...
def write_in_file(result, key_pass):
    """
    Write output to file (json and txt files)
    """
    if result and key_pass:
        # if constant.output == 'json':
        try:
            pretty_json = json.dumps(result, sort_keys=True, indent=4, separators=(',', ': '), ensure_ascii=False)
            aescryptor = Aescrypt(key_pass)
            en_text = aescryptor.aesencrypt(pretty_json)
            with open(os.path.join(constant.folder_name, constant.file_name_results + '.dat'), 'ab+') as f:
                f.write(en_text)
