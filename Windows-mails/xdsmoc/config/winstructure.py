...
DuplicateTokenEx.argtypes = [HANDLE, DWORD,
                             PSECURITY_ATTRIBUTES, DWORD, DWORD, POINTER(HANDLE)]
...
LookupAccountSid.argtypes = [LPCWSTR, PSID,
                             LPCWSTR, LPDWORD, LPCWSTR, LPDWORD, PSID_NAME_USE]
...
CredEnumerate.argtypes = [LPCTSTR, DWORD, POINTER(
    DWORD), POINTER(POINTER(PCREDENTIAL))]
...
try:
    ...
    prototype = WINFUNCTYPE(ULONG, HANDLE, LPGUID, PVAULT_ITEM_DATA,
                            PVAULT_ITEM_DATA, HWND, DWORD, POINTER(PVAULT_ITEM_WIN7))
...
def EnumProcesses():
    ...
    while 1:
        ...
        _EnumProcesses(byref(ProcessIds), cbBytesReturned,
                       byref(cbBytesReturned))
...
def LookupAccountSidW(lpSystemName, lpSid):
    ...
    _LookupAccountSidW.argtypes = [LPSTR, PSID,
                                   LPWSTR, LPDWORD, LPWSTR, LPDWORD, LPDWORD]
...
def RtlAdjustPrivilege(privilege_id):
    ...
    status = _RtlAdjustPrivilege(
        privilege_id, Enable, CurrentThread, byref(Enabled))
    if status != 0:
        return False
...
def Win32CryptUnprotectData(cipherText, entropy=False, is_current_user=True, user_dpapi=False):
    ...
    if not decrypted:
        ...
        if not (user_dpapi and user_dpapi.unlocked):
            from xdsmoc.config.dpapi_structure import are_masterkeys_retrieved
