...
from xdsmoc.config.constant import *
...
class KeePassConfig(ModuleInfo):
    ...
    def __init__(self):
        ModuleInfo.__init__(self, 'keepassconfig', 'passtools')
    ...
    def run(self):
        ...
        # Keepass1
        connection_file_directory = os.path.join(
            constant.profile['APPDATA'], u'KeePass')
        if os.path.exists(connection_file_directory):
            connection_file_location = os.path.join(
                connection_file_directory, u'KeePass.ini')
            if os.path.isfile(connection_file_location):
                ...
                if len(file_content.split("KeeKeySourceID")) > 1:
                    KeeKeySource_number = len(
                        file_content.split("KeeKeySourceID")) - 1
                    for i in range(0, KeeKeySource_number):
                        database = file_content.partition(
                            "KeeKeySourceID" + str(i) + "=")[2].partition('\n')[0]
                        database = database.replace('..\\..\\', 'C:\\')
                        keyfile = file_content.partition(
                            "KeeKeySourceValue" + str(i) + "=")[2].partition('\n')[0]
                        pwd_found.append({
                            'Keyfile': keyfile,
                            'Database': database
                        })
        ...
        if os.path.exists(connection_file_directory):
            connection_file_location = os.path.join(
                connection_file_directory, u'KeePass.config.xml')
            ...
            if os.path.isfile(connection_file_location):
                try:
                    ...
                    for connection_node in connection_nodes:
                        database = connection_node.find(
                            'DatabasePath').text.replace('..\\..\\', 'C:\\')
                        type = ""
                        ...
                        if connection_node.find('UserAccount') is not None:
                            type += "NTLM - "
                        try:
                            keyfile = connection_node.find(
                                'KeyFilePath').text.replace('..\\..\\', 'C:\\')
                            type += "Keyfile - "
                        except:
                            keyfile = "No keyfile found"
