...
from xdsmoc.config.change_privileges import list_sids, rev2self, impersonate_sid_long_handle
from xdsmoc.config.users import get_user_list_on_filesystem, set_env_variables, get_username_winapi
from xdsmoc.config.dpapi_structure import SystemDpapi, are_masterkeys_retrieved
from xdsmoc.config.execute_cmd import save_hives, delete_hives
from xdsmoc.config.write_output import print_debug, StandardOutput
from xdsmoc.config.constant import constant
from xdsmoc.config.manage_modules import get_categories, get_modules
...
def run_xdsmoc(category_selected='all', subcategories={}, password=None):
    """
    Execution Workflow:
    - If admin:
